...
private:
	Node<T>* root;//Êîðåíü
	std::function<bool(T, T)> func;//Ïîëèìîðôíàÿ îá¸ðòêà ôóíêöèè äîáàâëåíèÿ/ñîðòèðîâêè
	
	void srthelp(Tree<T>& tmpTree, Node<T>* ptr);//Ðåêóðñèâíàÿ ñîðòèðîâêà
	template<class T1>
	bool srchhelp(Tree<T>& tmpTree, const std::function<bool(T1, T)>& func, const T1& key, Node<T>* ptr);//Ðåêóðñèâíûé ïîèñê
	Node<T>* copyTree(Node<T>* ptr);//Ðåêóðñèâíîå êîïèðîâàíèå
	void print(Node<T>* p, int& index);//Ðåêóðñèâíûé âûâîä
	void size(int& sz, Node<T>* ptr);//Ðåêóðñèâíîå âû÷èñëåíèå ðàçìåðà
	T getElement(int index, Node<T>* ptr, T& element, bool& flag = false, int& counter = 0);
	void clearTree(Node<T>* ptr);//Ðåêóðñèâíàÿ î÷èñòêà äåðåâà
public:
